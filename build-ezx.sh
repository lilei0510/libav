...
$(dirname `readlink -f "$0"`)/configure --prefix=$ARM_INSTALL --host-cc=gcc --enable-cross-compile --cross-prefix=arm-linux- --target-os=linux \
 --sysinclude=$ARM_INSTALL/../include --ld=arm-linux-gcc --arch=armv5te --cpu=iwmmxt  \
